...
function GlassCard({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "bg-white/20 border-white/30 flex flex-col gap-6 rounded-2xl border py-6 backdrop-blur-xl shadow-2xl shadow-black/10",
        className,
      )}
      {...props}
    />
...
function GlassCardFooter({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("flex flex-col items-center px-5 [.border-t]:pt-6", className)}
      {...props}
    />
  )
}
